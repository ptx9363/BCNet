...
if __name__ == '__main__':
  ...
  if args.net == 'vgg16':
    fastRCNN = vgg16(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res101':
    fastRCNN = resnet(imdb.classes, 101, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res50':
    fastRCNN = resnet(imdb.classes, 50, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res152':
    fastRCNN = resnet(imdb.classes, 152, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'alexnet':
    fastRCNN = alexnet(imdb.classes, pretrained=True, class_agnostic=args.class_agnostic)
  else:
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois.data.resize_(data[4].size()).copy_(data[4])

      fastRCNN.zero_grad()

      rois, cls_prob, bbox_pred, \
      RCNN_loss_cls, RCNN_loss_bbox, \
      rois_label = fastRCNN(im_data, im_info, gt_boxes, num_boxes, rois)
    ...
    save_name = os.path.join(output_dir, 'fast_rcnn_{}_{}_{}.pth'.format(args.session, epoch, step))
    save_checkpoint({
