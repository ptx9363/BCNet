...
class resnet(_fastRCNN):
    def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False, pretrained_weight=''):
    ...
    def _init_modules(self):
        ...
        self.sbranch.RCNN_top = copy.deepcopy(self.RCNN_top)
        self.sbranch.RCNN_cls_score = nn.Linear(2048, self.n_classes)
        if self.class_agnostic:
            self.sbranch.RCNN_bbox_pred = nn.Linear(2048, 4)
        else:
            self.sbranch.RCNN_bbox_pred = nn.Linear(2048, 4 * self.n_classes)

        self.wbranch = copy.deepcopy(self.sbranch)
        self.sbranch2 = copy.deepcopy(self.sbranch)

        self.RCNN_base.apply(set_bn_fix)
        self.sbranch.RCNN_top.apply(set_bn_fix)
        self.wbranch.RCNN_top.apply(set_bn_fix)
        self.sbranch2.RCNN_top.apply(set_bn_fix)
    ...
    def train(self, mode=True):
        ...
        if mode:
            ...
            self.sbranch.RCNN_top.apply(set_bn_eval)
            self.wbranch.RCNN_top.apply(set_bn_eval)
            self.sbranch2.RCNN_top.apply(set_bn_eval)

    def _head_to_tail(self, pool5, branch):
        fc7 = branch.RCNN_top(pool5).mean(3).mean(2)
        return fc7
